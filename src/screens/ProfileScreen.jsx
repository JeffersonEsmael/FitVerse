    ...
    if (diff < 0 && pullProgress >= 1) {
      ...
      try {
        ...
        if (user?.uid) {
          ...
          if (profile?.profile_type === 'business' || profile?.profile_type === 'trainer') {
    ...
    <ScreenWrapper screenKey="profile">
      ...
      >
        ...
        <motion.div 
          style={{
            ...styles.profileCard,
            paddingTop: (p.profile_type === 'trainer' || p.profile_type === 'business') ? '140px' : '24px'
          }} 
        ...
        >
          {/* Cover Photo */}
          {(p.profile_type === 'trainer' || p.profile_type === 'business') && (
          ...
          <div className="profile-card-header" style={{ ...styles.profileCardHeader, position: 'relative', zIndex: 2 }}>
          ...
          <div style={{ ...styles.bioContainer, position: 'relative', zIndex: 2 }}>
            ...
            {/* Goals chips for personal profile */}
            {p.profile_type === 'personal' && p.fitness_goals?.length > 0 && (
            ...
            {p.profile_type === 'trainer' && (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', width: '100%', marginTop: '6px' }}>
                <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', justifyContent: 'center' }}>
                  {p.years_experience !== undefined && p.years_experience !== null && (
                    <span style={{ fontSize: '13px', color: '#00D4FF', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '4px' }}>
                      🏋️ {p.years_experience} {p.years_experience === 1 ? 'ano' : 'anos'} de carreira
                    </span>
                  )}
                  {p.students_count && (
                ...
                {Array.isArray(p.specialties) && p.specialties.length > 0 && (
                  <div style={{ ...styles.goalsCenter, marginTop: '4px' }}>
                    {p.specialties.map((spec) => {
                      ...
                      return (
                        <span key={spec} style={{ ...styles.goalChip, borderColor: '#00D4FF30', background: 'rgba(0,212,255,0.05)', color: '#00D4FF' }}>
        ...
        {activeProfileTab === 'sobre' && (p.profile_type === 'business' || p.profile_type === 'trainer') && (
          <div style={workoutStyles.container}>
            {p.profile_type === 'business' ? (
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
                  ...
                  <span style={styles.sobreValue}>{p.display_name}</span>
                </div>

                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
                  <span style={styles.sobreLabel}>🏋️ Carreira & Experiência</span>
                  <span style={styles.sobreValue}>
                    {p.years_experience} {p.years_experience === 1 ? 'ano' : 'anos'} atuando como Personal Trainer
                  </span>
                </div>
                ...
                {p.students_count && (
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
                    ...
                    <span style={styles.sobreValue}>{p.students_count}</span>
                  </div>
                )}
                ...
                {p.certifications && (
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
                    ...
                    <span style={styles.sobreValue}>{p.certifications}</span>
                  </div>
                )}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', width: '100%', marginTop: '8px' }}>
                  ...
                  {p.whatsapp ? (
                    <motion.a
                      whileTap={{ scale: 0.97 }}
                      href={`https://wa.me/${p.whatsapp}?text=${encodeURIComponent('Olá, vi seu perfil no FlowRise e gostaria de agendar uma consultoria!')}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      style={styles.whatsappBtn}
                    >
            ...
            <div style={styles.feedbacksHeader}>
              <h4 style={styles.feedbacksTitle}>
                {p.profile_type === 'trainer' ? 'Feedbacks de Alunos' : 'Feedbacks de Frequentadores'}
              </h4>
